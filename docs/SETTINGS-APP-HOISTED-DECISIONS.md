...
| **Diagnostics-Export** | `onExportDiagnostics` schreibt die Diagnose-ZIP in den festen Ordner `Download/Vivicast/Diagnostics`; Crash-Logging (`CrashLogWriter`) ist App-hoisted, immer aktiv. (Support-Copy + separater Einstellungs-Export wurden entfernt.) | Context/Dateisystem | Belassen; Diagnostics-**DataStore** ist bereits im VM (f2a) |
