...
| Appearance (Theme/Accent/Transparency/FontScale, Language-Pref) | ✅ vollständig | `UserPreferencesStore` | f1; Locale/`recreate()` bleibt App |
...
|--------|-------------------|---------|----------------------------|
| **Backup** (`BackupSettingsPanel`) | In-App-Datei-Picker (`FilePickerDialog`/`StorageAccess`, SAF-Ersatz), `StandardBackupExporter/Restorer`, Passphrase-`CharArray` (Export- + Import-Passphrase-Dialog in `AppDialogs`), Protection-Unlock. Panel triggert nur `onExportBackup()`/`onImportBackup()`; State/Ablauf kommen aus `MainActivity`. | Picker/Permission/Security an Activity-Lifecycle gebunden; VM-Migration brächte kaum Nutzen, hohes Risiko | Nur falls separates Backup-/Security-Konzept kommt; sonst belassen |
...
| **Diagnostics-Export** | `onExportDiagnostics` schreibt die Diagnose-ZIP in einen per `FilePickerDialog` gewählten Ordner (letzter Ordner in `diagnostics.lastExportDir` gemerkt; fester `Download/Vivicast`-Fallback via `writeToUserFolderOrFallback`); Crash-Logging (`CrashLogWriter`) ist App-hoisted, immer aktiv. (Support-Copy + separater Einstellungs-Export wurden entfernt.) | Context/Dateisystem/Picker | Belassen; Diagnostics-**DataStore** ist bereits im VM (f2a) |
| **Diagnostics-Event-Logging** (`onLogProviderSaved`/`onLogProviderDeleted`/`onLogGroupEvent`) | App-Layer schreibt in `DiagnosticsStore` (gegated durch den Logging-Toggle); Feature/VM fassen den Store nie an — sie melden nur über diese Callbacks. Provider/Category-ids gehen als opake `target`-Werte (Random-UUID / gehashte category-id) rein. Deckt provider `saved`/`deleted` + die group-events (hidden/shown, bulk, reorder, order_reset, sort_mode_changed, new_groups_policy_changed) ab. | Diagnostics-Store ist App-owned | Belassen |
...
| **M3U-Dateipicker** (`onPickM3uFile`) | In-App `FilePickerDialog` + `StorageAccess` (SAF-Ersatz; `File.listFiles()` + Runtime-/All-Files-Permission). | Datei-/Permission-Zugriff am Activity-Lifecycle | Belassen |
