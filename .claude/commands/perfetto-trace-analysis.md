...
- **Domain Hints:** Reference files for specific performance areas: [`CPU`](perfetto-trace-analysis/references/hints_cpu.md), [`Graphics`](perfetto-trace-analysis/references/hints_graphics.md), [`I/O`](perfetto-trace-analysis/references/hints_io.md), [`IPC`](perfetto-trace-analysis/references/hints_ipc.md), [`Memory`](perfetto-trace-analysis/references/hints_memory.md), [`Power`](perfetto-trace-analysis/references/hints_power.md). These files each contain multiple expert-vetted, powerful trace analysis techniques to steer and aid in the analysis.
- **Perfetto SQL Reference:** Reference guidelines for translating intents into valid queries are located in [the SQL reference](perfetto-trace-analysis/references/sql.md). You must read this reference and follow its Execution Protocol for all SQL generation.
...
3. **Review SQL Reference:** Read the SQL reference in [`references/sql.md`](perfetto-trace-analysis/references/sql.md) and follow its Execution Protocol for all SQL generation. Do not guess schemas.
...
- **Prioritization:** Form hypotheses using information from: user prompt \> "Domain Hints" ([`CPU`](perfetto-trace-analysis/references/hints_cpu.md), [`Graphics`](perfetto-trace-analysis/references/hints_graphics.md), [`I/O`](perfetto-trace-analysis/references/hints_io.md), [`IPC`](perfetto-trace-analysis/references/hints_ipc.md), [`Memory`](perfetto-trace-analysis/references/hints_memory.md), [`Power`](perfetto-trace-analysis/references/hints_power.md)) \> general knowledge. Be sure to leverage these "Domain Hints" as they are expert-vetted analysis techniques.
...
- **Multiple Bottlenecks:** Complex performance issues rarely have a single cause. Do NOT stop your investigation after finding the first anomaly. Even if you find a major bottleneck (e.g., emulator graphics lag), you MUST continue searching for other independent system-wide issues (e.g., lock contention, I/O stalls). To find other bottlenecks, search through the content of the "Domain Hints" files ([`CPU`](perfetto-trace-analysis/references/hints_cpu.md), [`Graphics`](perfetto-trace-analysis/references/hints_graphics.md), [`I/O`](perfetto-trace-analysis/references/hints_io.md), [`IPC`](perfetto-trace-analysis/references/hints_ipc.md), [`Memory`](perfetto-trace-analysis/references/hints_memory.md), [`Power`](perfetto-trace-analysis/references/hints_power.md)) to retrieve and leverage expert-vetted, powerful trace analysis techniques. Investigate each relevant hint with depth.
